...
exports.getUserData = async (req, res) => {
    try {
        ...
        if (userType === 'user') {
            ...
            const userQuery = 'SELECT user_status FROM users WHERE user_id = $1 AND institute_id_c = $2';
            const userResult = await db.query(userQuery, [userID, instituteID]);

            if (userResult.rows.length === 0) {
                return res.status(404).json({ message: 'User ID not found in this institute' });
            }
            ...
            const instituteDataResult = await db.query(instituteDataQuery, [instituteID]);
            const data = instituteDataResult.rows;

            return res.status(200).json({ message: 'Institute data fetched successfully', data });
        }
    } catch (error) {
        console.error('Error processing request:', error);
        return res.status(500).json({ error: 'Internal Server Error' });
