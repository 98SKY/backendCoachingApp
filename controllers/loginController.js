...
exports.verifyOtp = async (req, res) => {
  try {
    ...
    if (userType === "institute") {
      verifyQuery =
        "SELECT * FROM institutes WHERE email = $1 AND authenticate = $2";
      verifyParams = [email, otp];
    } else if (userType === "student" || userType === "teacher") {
      verifyQuery =
        "SELECT * FROM users WHERE email = $1 AND authenticate = $2";
      verifyParams = [email, otp];
    } else {
  ...
  try {
    ...
    if (accountType === "institute") {
      ...
      if (email !== instituteEmail) {
        return res
          .status(400)
          .json({ error: "Email not registered with username" });
      }

      // Update password
      const hashedPassword = await bcrypt.hash(newPassword, 10);
      const updateQuery =
        "UPDATE institutes SET password = $1 WHERE username = $2";
      ...
      if (email !== userEmail) {
        return res
          .status(400)
          .json({ error: "Email not registered with this user" });
      }
